...
const amazon_bookname = () => {
  ...
  const dest = HOST + encodeURI(bookName);

  return dest;
};

const goodreads_addbtn = (fetchBookName) => {
  const ul = document.querySelector(".BookActions");
  if (ul) {
    const btn = createButton();
    btn.addEventListener("click", () => openNewTab(fetchBookName));
    ul.prepend(btn);
...
const goodreads_bookname = () => {
  const bookName = document.querySelector("h1.Text__title1").textContent.trim();
  const dest = HOST + encodeURI(bookName);
  return dest;
};
