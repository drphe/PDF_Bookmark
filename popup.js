...
const extpay = ExtPay("pdf-bookmark");
  ...
  .then((user) => {
    if (user.paid) {
      document.getElementById("payment").innerHTML =
        'Premium <i class="fas fa-check-circle"></i>';
    }
  })
  .catch((err) => {
    document.getElementById("payment").innerHTML =
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var info = document.getElementById("info-icon");
  info.addEventListener("click", function () {
    $("#info-premium").slideToggle();
  });
  ...
  deleteAll.addEventListener("click", function () {
    // Confirmation window
    var confirmed = confirm("Are you sure of removing this bookmark?");
    if (confirmed) {
      // Delete everything
      $("#accordion").empty();
  ...
  addBook.addEventListener("click", function () {
    add = true;
    addGroup = false;
    // Gets the current URL and callback the storage system.
    chrome.tabs.query(
      ...
      function (tabs) {
        ...
        if (url.includes(".pdf")) {
          ...
          // Calls the storage API from google to get all the items saved
          // "bookmarks" is the key of the JSON object that holds all of the data
          chrome.storage.sync.get("bookmarks", function (items) {
            extpay
              .getUser()
              .then((user) => {
                if (items.bookmarks === undefined ||
                  items.bookmarks === null || items.bookmarks.length < 3 || user.paid) {
                  // If it is empty
                  if (
                    ...
                    for (key in items.bookmarks) {
                      ...
                      if (
                        ...
                        if (!items.bookmarks[key].page.includes(pageNumber)) {
                          ...
                          alert(
                            "This bookmark in the book: " +
                              book_name +
                              " already exists in your bookmarks list"
                          );
                          // We won't add it
                          add = false;
                          break;
                    ...
                    //If it doesn't exist we will add it to our list of bookmarks
                    if (add) {
                      // Appends the new bookmark to the JSON object
                      ...
                      // Calls the API to replace all the JSON object with the JSON .
                      chrome.storage.sync.set({
                        bookmarks: items.bookmarks,
                      });
                      // Add the bookmark to the HTML list.
                      reloadData();
                    }
                  }
                } else{
                  alert("You only can have maximum 3 bookmarks without premium. Upgrade your account to get unlimited bookmarks and more features!")
                }
              })
              .catch((err) => {
                alert(
                  "Error we can't connect to internet :( or get your account premium info."
                );
              });
          });
        } else {
          //Todo when it is not PDF.
        }
      }
    );
  });
...
/*================== LOAD ALL STORED DATA ON OPENING ================== */
...
function addItemToListGroup(path, pages, book, date) {
  ...
  for (page in orderedBookmark) {
    groupHTML =
      ...
      '<div class="col-6 text-muted pt-2">Created on: ' +
